...
class _QemuVirtImage(_VirtImage):
    ...
    def _define_config_legacy(cls, image_name, image_params, node_tags):
        config = super()._define_config_legacy(image_name,
                                               image_params,
                                               node_tags)
        config["spec"]["volume"] = define_resource_config("volume",
                                                          image_params,
                                                          node_tags)
        return config
    ...
    def create_object(self):
        LOG.debug(f"Create the volume object for '{self.virt_image_name}'")
        volume_config = self.virt_image_spec["volume"]
        create_resource_object(volume_config)

    def destroy_object(self):
        LOG.debug(f"Destroy the volume object for '{self.virt_image_name}'")
